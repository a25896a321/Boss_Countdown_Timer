# Countdown_Timer_Mini_v1.0
![Version](https://img.shields.io/badge/Version-v1.1-blue)
![OS](https://img.shields.io/badge/OS-Windows-green)
![License](https://img.shields.io/badge/License-Copyright-red)

> **此為迷你精簡版！此為迷你精簡版！此為迷你精簡版！**

該程式是一款專為遊戲輔助設計的倒數計時工具，支援多組獨立計時、自定義音效及透明介面，幫助玩家精準掌控 BOSS 機制與技能循環。

This program is a precise countdown tool designed for gaming assistance. It supports multiple independent timers, custom sound effects, and a transparent interface, helping players master BOSS mechanics and skill cycles.

> **💡 詳細說明請看壓縮檔內使用指南**

---

## 🛡️ 重要設定 (Important Setup)

為了確保快捷鍵與置頂功能正常運作，請務必設定**系統管理員權限**：

* **方法 1：** 右鍵點擊程式執行檔 → 選擇「以系統管理員身分執行」。
* **方法 2：** 右鍵點擊程式執行檔 → 內容 → 相容性 → 勾選「以系統管理員身分執行此程式」。

---
**簡易使用流程：**
* 火眼召喚出特效：`Num /`
* 本體切換模式：`Tab`
* 遇到機制魔方重置：`` ` `` (重音符號鍵)
* 結束關閉：`Num *`

**⚠️ 實戰技巧：**
* 火眼召喚按 " / "
* 進本體按 " TAB "
* 遇到機制魔方按 " ` "
* 本體 3 階建議減少手動加時動作。
* 本體 3 階的機制魔方為單顆形式。

---
## 🖥️ 功能說明 (Features)
* **置頂及語言 (Stay on Top & Language)：** 設定置頂避免被擋住、調整中英語言。
* **起始計時器 (Start Timer)：** 亮綠色顯示，適合固定間隔事件，具備自動循環功能。
* **回合計時器 (Round Timer)：** 雙階段切換。回合 1（粉紫色）結束後，自動切換至回合 2（亮綠色）。
* **透明度 (Transparency)：** 透過拉桿可調整 5% - 100% 透明度。

---

## 🙏🏽 感謝支持

如果這個程式對您有幫助，歡迎點擊程式內的**「贊助訂閱作者 YouTube 粉圓jump遊戲頻道」**給予支持！

---

## 【 版權與免責聲明 / License & Disclaimer 】

**Copyright © 2026 _oojump. All rights reserved.**
本程式僅供個人使用，未經許可請勿商用或二次公開發布。

1. 本程式僅提供倒數計時與視覺提醒功能，**不含任何自動化操作、未修改遊戲、無模擬腳本或自動化模組**。
2. 本程式**不會讀取、修改遊戲記憶體**，亦不具備任何影響遊戲數據之功能。
3. 使用本程式不具備外掛風險，惟使用者仍應遵守各遊戲之使用者協議，開發者對於使用本程式所產生的任何法律或帳號爭議不負任何責任。

*(This program provides only countdown and visual notification features. It does not use any automation modules, simulation scripts, or automation packages. The program does not read or modify game memory, nor does it possess the ability to alter game data or perform automated in-game actions. While using this program does not pose a hacking risk, users should still comply with the Terms of Service of their respective games. The developer shall not be held liable for any legal or account-related disputes arising from the use of this software.)*

---
**Developer:** oo_jump | **OS:** Windows | **Version:** Mini_v1.0
